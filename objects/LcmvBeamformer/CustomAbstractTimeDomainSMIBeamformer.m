classdef (Hidden) CustomAbstractTimeDomainSMIBeamformer < CustomAbstractTimeDomainBeamformer
...
%#codegen
    ...
    properties (Nontunable) 
        ...
        %   property is 1.
        FilterLength (1, 1) {mustBePositive, mustBeInteger} = 1;
        ...
        TrainingInputPort (1, 1) logical = false;
    ...
    methods (Access = protected)
        function obj = CustomAbstractTimeDomainSMIBeamformer(varargin)
            obj@CustomAbstractTimeDomainBeamformer(varargin{:});
        end
    end
    
    methods (Access = protected)
        function validatePropertiesImpl(obj)
            validatePropertiesImpl@CustomAbstractTimeDomainBeamformer(obj);
        end
        
        function setupImpl(obj,x,varargin)
            setupImpl@CustomAbstractTimeDomainBeamformer(obj,x);
        ...
        function flag = isInputComplexityMutableImpl(obj,index) 
            flag = isInputComplexityMutableImpl@CustomAbstractTimeDomainBeamformer(obj,index);
        ...
        function releaseImpl(obj)
            releaseImpl@CustomAbstractTimeDomainBeamformer(obj);
            release(obj.cWeightsEstimator);
        end
        
        function resetImpl(obj)
            resetImpl@CustomAbstractTimeDomainBeamformer(obj);
            reset(obj.cWeightsEstimator);
            obj.pDataBuffer(:) = 0;
        end

        function num = getNumInputsImpl(obj)
            num = getNumInputsImpl@CustomAbstractTimeDomainBeamformer(obj);
            if obj.TrainingInputPort
                num = num + 1;
            end
        end
        ...
        function num = getNumOutputsImpl(obj)
            num = getNumOutputsImpl@CustomAbstractTimeDomainBeamformer(obj);
        ...
        function validateInputsImpl(obj,x,varargin)
            validateInputsImpl@CustomAbstractTimeDomainBeamformer(obj,x);
        ...
        function s = saveObjectImpl(obj)
            s = saveObjectImpl@CustomAbstractTimeDomainBeamformer(obj);
            if isLocked(obj)
        ...
        function s = loadSubObjects(obj,s)
            s = loadSubObjects@CustomAbstractTimeDomainBeamformer(obj,s);
        ...
        function spaceTimeSnap = prepareSpaceTimeSnapshot(obj,x,streamflag)
            ...
            if streamflag
                ...
                obj.pDataBuffer = x_augmented(M+(1:filtlen-1),:);
            else
                for m = 1:filtlen
    ...
    methods (Access = protected) %for Simulink
        function varargout = getInputNamesImpl(obj)
            %Insert XT
            if obj.TrainingInputPort
                [varargout{1:nargout-1}] = getInputNamesImpl@CustomAbstractTimeDomainBeamformer(obj);
                varargout = {varargout{1} 'XT' varargout{2:end}};
            else
                [varargout{1:nargout}] = getInputNamesImpl@CustomAbstractTimeDomainBeamformer(obj);
            end
        end
        function varargout = getOutputSizeImpl(obj)
            [varargout{1:nargout}] = getOutputSizeImpl@CustomAbstractTimeDomainBeamformer(obj);
            if obj.WeightsOutputPort
                szW = varargout{2};
